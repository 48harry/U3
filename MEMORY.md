...
- 베이스라인(IsolationForest, 2026-09-23 재학습) 검증 결과: 전체 AUROC 0.630, 고장 전 1h recall 4.4%, 정상 알람률 1.4%. **기준선일 뿐이고 종료 상태(E029 등)도 못 잡는다.**
...
- 베이스라인 테스트 결과(8파일, 80,533 윈도우): AUROC 0.687, recall@1h 5.4%, 정상 알람률 3.2%(검증 1.4%의 두 배 — 건 분포 차이, 특히 E02 11%). 클래스별 AUROC E01 0.45 / E02 0.80 / E03 0.89 / E04 0.68. 검증과 같은 결론: 기준선.
...
- 서빙은 번들의 `preprocess_config`에서 gap 한도·c16 임계를 읽고(없으면 기본값 + 경고), `per-file` 스케일이나 `--resample`로 만든 번들은 기동 시 거부한다 — 온라인으로 재현할 수 없는 변환이라 조용히 틀린 점수를 내느니 실패하는 쪽을 택했다.
...
- `pytest`(= `tests/test_pipeline.py`)는 합성 CSV로 전처리→테스트 분할→학습→평가→API까지 임시 폴더에서 돌린다(~15초). 파이프라인 코드를 고치면 먼저 이걸 돌린다. 실데이터 폴더는 건드리지 않는다.
...
4. 다음 실험 후보: per-file 스케일 + static 제거(단, per-file은 서빙이 거부하므로 서빙용 스케일 전략을 같이 정해야 함), 윈도우 확대(5~10분), 시퀀스 오토인코더, `label`을 쓰는 지도학습 비교, E02의 수집 시기 분포 차이 확인.
5. 테스트 8파일은 클래스 추론 결과 E01×2, E02×1, E03×2, E04×3, 결측 0.7~2%로 모두 유지된다. 단 **test_2는 행의 95%, test_3은 87%가 `non_welding`**(c16 규칙) — 학습 파일 최대치(45%)를 훨씬 넘는다. c16 규칙이 이 두 건에서는 "캡 드레싱"이 아니라 다른 setpoint 관행일 수 있으니, 결과 해석 전에 c16 분포를 확인해야 한다.
